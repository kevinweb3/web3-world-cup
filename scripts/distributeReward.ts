...
function generateMerkelTree(index: number, playerRewardList: any) {
  // make leafs
  let items = playerRewardList.map((it: { rewardAmt: number; player: string; }) => {
    console.log('it.rewardAmt:', it.rewardAmt);
    return generateLeaf(index, it.player, it.rewardAmt);
  })

  // create tree
  const tree = new MerkleTree(items, hre.ethers.keccak256, { sort: true })
  return tree
}
...
async function main() {
  ...
  const player = playerDistributions.filter(function (item: { player: string; }) {
